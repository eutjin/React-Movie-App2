...
function AddList({ movieId, variable }) {
  ...
  const handleAddtoList = async (listid) => {
    ...
    console.log(
      list
        .filter((list) => list.list._id == listid)
  ...
  const removeFromList = async (listid) => {
    const deleteId = list
      .filter((list) => list.list._id == listid)
      .find((list) => list.id == movieId)._id;
    console.log("deleteId", deleteId);

    const response = await axios.delete(
      baseUrl + "/api/list/removeFromList/" + deleteId
    );
    console.log(response.data);
    ...
    <div>
      ...
      >
        ...
        {listTitles.map((list1) => (
          <Menu.Item onClick={() => handleAddtoList(list1._id)}>
            <div className={classes.list}>
              <div className={classes.listTitle}>{list1.listTitle}</div>
