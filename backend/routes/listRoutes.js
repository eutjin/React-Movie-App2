...
router.post("/getAllList", (req, res)=>{
    ...
    // console.log(req.body)
    ListType.find({'user':req.body.user}).populate('user').populate('listContents').exec((err, lists)=>{
        if(err) return res.status(400).send(err)
...
router.post("/addToList", (req, res)=>{
  ...
  customlist.save((err, list)=>{
    ...
    //     console.log(result)
    // })
    ListType.updateOne({_id: req.body.list}, {$push: {listContents: list._id}}, {new: true}).exec((err, result)=> {
      if (err) return res.json({ success: false, err });
